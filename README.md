...
- 暂不做翻仓、借券、做空产品规则和真实交易通道；撤单链路已接到 adapter，真实通道实现后复用同一接口。
...
- 异步撤单：`CancelBasket`、`CancelAll` 会查找在途订单并调用 `VenueAdapter::cancel_order`；撤单成功后由 `Canceled` 回报统一释放未成交 reservation。
- 撤单拒绝：支持 `CancelRejected` 回报，订单会回到 `New` / `PartiallyFilled`，不会释放 reservation。
- 撤单候选：只把 `New` / `PartiallyFilled` 等可撤订单交给 adapter；`PendingCancel` 中的重复撤单会幂等处理，不误报失败。
...
- CTest 验证：已拆成状态机、订单表、状态视图、报价、planner、撤单流程和异步集成测试；覆盖 ack-only 后异步全成、异步拒单释放资源、部分成交后撤单重规划、CancelBasket、CancelAll、撤单拒绝、重复撤单幂等、venue order id 反查、重复成交幂等、overfill 截断、盘口报价模型、显式限价不被覆盖和现金裁剪。
...
Halted    禁止新子单，撤单通道仍可用
Killed    kill switch 触发后的终态，需要人工恢复
```

当前 `ControlCommand` 已能切换 `Pause -> Halted`、`Resume -> Active`、`ReduceOnly -> Reducing`、`KillSwitch -> Killed`，也支持 `CancelBasket` 和 `CancelAll` 发起异步撤单。
...
1. 抽出 `IMarketDataView`，把盘口视图从 `ExecutionStateView` 里拆出来，兼容内置行情库和外部行情服务。
2. 增加 `VenueMapper`，把内部 `BUY/SELL + LONG/SHORT + quantity` 映射到不同交易所字段。
3. 增加 instrument / venue metadata：是否可做空、最小名义金额、tick size、lot size、涨跌停、交易时段、保证金、资金费率。
4. 增加真实交易通道 adapter，包括发单、撤单、查询和 venue order id 映射。
5. 增加外部快照 reconciliation：订单、成交、持仓、资金快照和本地 overlay 的对账。
6. 增加异步 journal / snapshot 旁路；热路径仍只写内存，持久化不能阻塞发单和回报处理。
7. 性能优化：instrument 字符串换成内部整数 ID，调仓 / intent / order 预分配，热路径去日志字符串，事件循环改成无锁或少锁队列。
